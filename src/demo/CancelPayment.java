	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flag.equals("del"))
		{
			try
			{
				...
				ResultSet rs = stmt.executeQuery("delete from booth_payment where t_id="+id);
				ResultSet rs1 = stmt.executeQuery("delete from transaction where t_id="+id);
			
				System.out.println("delete: "+id);
				response.setContentType("text/javascript");
				PrintWriter out=response.getWriter();
				out.print("<script>alert('Payment cancel successfully...');</script>");
